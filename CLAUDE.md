...
배터리 잔량 10% 이하에서만 입장 가능한, 국가별로 매칭되는 2D(손그림 룸 배경 + 통통한 SD 동물 캐릭터, 큐트-흑화 카툰 스타일) 메타버스 시한부 라운지. 상세 기획은 `docs/기획서.md`가 유일한 소스다 — 이 문서는 "레포를 어떻게 다루는가"만 다룬다.
...
- **비주얼 에셋은 스크립트 생성 자리표시자** — 라운지 배경(`apps/mobile/assets/background`)과 캐릭터(`apps/mobile/assets/characters`)는 기획서 2.1.1의 큐트-흑화 팔레트에 맞춰 스크립트로 그린 자리표시자다(각 폴더 `CREDITS.md`에 규격·생성 방법 기록). char_01~char_12 전원이 같은 자리표시자 캐릭터를 공유한다. Higgsfield로 뽑은 최종 아트로 교체하기 전까지의 자리표시자다 (기획서 9.4, 실행체크리스트 0단계)
...
- **제품 동작이 바뀌면 `docs/기획서.md`도 반드시 같이 고친다.** 상태머신 전이·화면 흐름·진입 조건·소켓 이벤트처럼 기획에 해당하는 것을 코드에서만 바꾸면 기획서가 거짓이 된다. 순서는 기획서 → SKILL.md(및 AGENTS.md 요약) → 코드.
- **변경 이력은 문서에 남기지 않는다.** "원래는 X였는데 바꿨다", "기획서엔 없지만 구현에서 보충했다", 날짜가 박힌 버그 수정 메모 같은 로그성 서술을 문서·README·코드 주석 어디에도 쓰지 않는다 — 이력은 git이 갖고 있다. 단 "왜 이렇게 되어 있는가"(설계 의도)는 과거 사건이 아니라 현재의 인과로 남긴다. 상세는 SKILL.md "문서는 현재 사양만 적는다" 절 참고.
